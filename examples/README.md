...
At first, the model has to be defined. This is realized by implementing a class that derives from `tl.model.AbstractModel`. This class provides a standard interface to define the graph structure, the loss layer, as well as how this model has to be evaluated by the framework.
...
with tl.core.MultiGpuRuntime("/tmp/train", gpu_devices=[0, 1]) as rt:
  rt.register_model(ConvAutoencoderModel(5e-4))
  rt.register_optimizer(tl.training.Optimizer("adam", initial_lr=0.001,
...
```python
with tl.core.DefaultRuntime("/tmp/train") as rt:
